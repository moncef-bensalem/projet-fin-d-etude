...
export default function ManagerSellersPage() {
  ...
  useEffect(() => {
    if (user && user.role === 'MANAGER') {
      console.log('Manager user detected, fetching sellers...');
      fetchSellers();
    }
  }, [user]);
  ...
  const handleApprove = async (seller) => {
    try {
      ...
      if (confirm(`Êtes-vous sûr de vouloir approuver le vendeur ${seller.name} ?`)) {
        setIsLoading(true);
        ...
        setTimeout(() => {
          ...
          setIsLoading(false);
          toast.success(`Le vendeur ${seller.name} a été approuvé avec succès`);
        }, 1000);
      }
    } catch (error) {
  ...
  const handleReject = async (seller) => {
    if (confirm(`Êtes-vous sûr de vouloir rejeter le vendeur ${seller.name} ?`)) {
      try {
        setIsLoading(true);
        ...
        setTimeout(() => {
          ...
          setSellers(updatedSellers);
          setIsLoading(false);
          toast.success(`Le vendeur ${seller.name} a été rejeté`);
        }, 1000);
      } catch (error) {
        console.error("Erreur lors du rejet:", error);
        toast.error("Une erreur est survenue lors du rejet du vendeur");
        setIsLoading(false);
      }
    }
  };
  ...
  const handleViewDetails = (seller) => {
    router.push(`/manager/dashboard/sellers/${seller.id}`);
  };
